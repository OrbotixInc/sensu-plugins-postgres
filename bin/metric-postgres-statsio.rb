...
class PostgresStatsIOMetrics < Sensu::Plugin::Metric::CLI::Graphite
  ...
  def run
    ...
    con.exec(request.join(' ')) do |result|
      result.each do |row|
        output "#{config[:scheme]}.statsio.heap_blks_read", row['heap_blks_read'], timestamp
        output "#{config[:scheme]}.statsio.heap_blks_hit", row['heap_blks_hit'], timestamp
        output "#{config[:scheme]}.statsio.idx_blks_read", row['idx_blks_read'], timestamp
        output "#{config[:scheme]}.statsio.idx_blks_hit", row['idx_blks_hit'], timestamp
        output "#{config[:scheme]}.statsio.toast_blks_read", row['toast_blks_read'], timestamp
        output "#{config[:scheme]}.statsio.toast_blks_hit", row['toast_blks_hit'], timestamp
        output "#{config[:scheme]}.statsio.tidx_blks_read", row['tidx_blks_read'], timestamp
        output "#{config[:scheme]}.statsio.tidx_blks_hit", row['tidx_blks_hit'], timestamp
      end
    end
